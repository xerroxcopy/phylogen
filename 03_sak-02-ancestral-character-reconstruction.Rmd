...
# reconstruct 71 traits
sak_phyDat_modern |> as.character()
sak_acctran <- acctran(tree = sak_nj_modern, data = sak_phyDat_modern) # 55 sequences with 71 character and 58 different site patterns.
...
try reconstructing 71 chars, not 58 unique site patterns
...
有根系統樹にして、オリジナルを探したいが、オリジナルがどこにあるかはわからない想定でやっている。（おそらく)ggtreeのyscale = weightとかは中点を使っている。そのため、（おそらく）中点を使う。どこを中点とみなしたかはプロットしてみないとわからないので、03_sak-03-ancestral-plot.Rmdをつかってどこのnodeかを確かめる。
...
<https://www.phytools.org/eqg2015/asr.html> "discrete characters"
