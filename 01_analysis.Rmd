---
title: "Plots in A Paper Submitted for AIIT Bulletin (14)"
output: html_notebook
---

# libraries

ライブラリをロード

```{r echo=FALSE, message=FALSE, results='hide'}
# library(MASS) # corresp analysis for sak. load before tidyverse so that it won't mask select from dplyr
library(tidyverse)
library(googlesheets4)
# library(lubridate)
library(fontregisterer)
# library(ggridges)
library(ggrepel)
# library(sf) # for anthro map
# library(rnaturalearth) # for anthro map
# library(rnaturalearthdata) # for anthro map
library(ape) # https://www1.doshisha.ac.jp/~mjin/R/Chap_28/28.html
# library(poppr) # https://grunwaldlab.github.io/poppr/reference/upgma.html
# library(ggdendro) # https://cran.r-project.org/web/packages/ggdendro/vignettes/ggdendro.html
library(FactoMineR) # for MCA(), not mca()
library(factoextra) # fviz_cluster
# https://bioconductor.org/packages/release/bioc/html/ggtree.html
# if (!requireNamespace("BiocManager", quietly = TRUE))
#     install.packages("BiocManager")
# 
# BiocManager::install("ggtree")
library(phangorn) # neighbornet
library(ggtree)
# library(cultevo) # hamming distance ("uncorrectedP" in splitsTree4) not used. use e1071 (faster) and phangorn (for phyDat)
# names(quartzFonts())
# remotes::install_github("KlausVigo/ggnetworx")
library(ggnetworx) # plot networx. is this identical to tanggle?
# if (!requireNamespace("BiocManager", quietly = TRUE))
#     install.packages("BiocManager")
# 
# BiocManager::install("tanggle")
# library(tanggle) # https://klausvigo.github.io/tanggle/articles/tanggle_vignette.html
# library(RRphylo) # https://cran.r-project.org/web/packages/RRphylo/vignettes/RRphylo.html
library(cowplot) # ggally??
library(e1071) # for hamming
library(phytools)
library(aplot)

```

```{r}
setwd("~/Documents/phylogen")
theme_set(theme(text = element_text(family = "YuGothic")))
knitr::opts_chunk$set(dev = "ragg_png")
```


# data

データは[Google Spreadsheets](https://docs.google.com/spreadsheets/d/1VZzjCMs21iBTIXoHR2Ph8wqyS8jQBlS4TDAmJXYjVKI/edit?usp=sharing)に格納している。データの各所が「どうしてこうなっているか」を忘れている部分があるが、仕方ない。なんとかしたい。

```{r}
data_anth_raw <- read_sheet("https://docs.google.com/spreadsheets/d/1VZzjCMs21iBTIXoHR2Ph8wqyS8jQBlS4TDAmJXYjVKI/edit?usp=sharing", sheet = "anthropomorphic artefacts")
data_sak_raw <- read_sheet("https://docs.google.com/spreadsheets/d/1CMR07q3v-0UwosNtEHKAyekOpQTRj7dpLUb323dFt-E/edit#gid=0", sheet = "swiss army knives")
data_idol_raw <- read_sheet("https://docs.google.com/spreadsheets/d/1VZzjCMs21iBTIXoHR2Ph8wqyS8jQBlS4TDAmJXYjVKI/edit?usp=sharing", sheet = "idol lightsticks")
data_sak_raw_old <- read_sheet("https://docs.google.com/spreadsheets/d/1Spgc75_EZV1eVJkGVIdzKFlbSIWLxXzeDB1gazu2Res/edit?usp=sharing", sheet = "swiss army knives")
```



