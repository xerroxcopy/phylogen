...
sak_nj3 <- full_join(sak_nj_modern, df_sak_nj_modern, by = "node") |> 
  mutate(id = if_else(is.na(id), paste0("node", node), id)) # unrooted
...
```{r}
df_sak_nj_modern |> 
 mutate(id = if_else(is.na(id), paste0("node", node), id)) |> 
...
```{r}
df_sak_old <- df_sak |> filter(str_detect(short_name, pattern = "OLD")) 
...
ggsave(filename = "output/sak_pheno.pdf", 
       ...
       dpi = 450,
       unit = "mm",
       height = 190, # pixelsでのサイズ
       width = 166,  # pixelsでのサイズ
       limitsize = FALSE)
```
