...
```{r}
sak_upgma_ace_modern <- full_join(sak_upgma_modern, df_sak_upgma_modern, by = "node")
sak_nj_ace_modern <- full_join(sak_nj_modern, df_sak_nj_modern, by = "node") # unrooted
...
sak_nj_ace_modern |> 
  ...
  geom_text_repel(aes(label = node), min.segment.length = .3) +
...
ggsave(filename = "output/sak_modern_pheno_noPhenoCategory_other_than_layers.pdf", 
       device = cairo_pdf,
       dpi = 450,
       unit = "mm",
       height = 300, 
