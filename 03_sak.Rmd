...
df_sak_corresp2 <- bind_cols(df_sak, df_sak_mca_rs)
```
...
```{r}
df_sak_corresp2 |>
  ggplot(aes(mca_x *1e5, mca_y * 1e5, label = short_name, colour = weight)) +
...
```{r}
sak_dist_jaccard <- dist(df_sak_corresp, method = "binary")
sak_upgma_jaccard <- sak_dist_jaccard |>
  hclust(method = "average")  # average = UPGMA
sak_upgma_tree_jaccard <- sak_upgma_jaccard |> 
   as.phylo()
sak_upgma_tree_jaccard$tip.label <- paste0(df_sak_corresp2$class, df_sak_corresp2$short_name)
...
```{r}
sak_dist <- hammingdists(df_sak_corresp) # 5sec. sometimes it's finished very fast but it means it's not running...? in that case, restart R
...
sak_upgma_tree$tip.label <- df_sak$short_name
```
...
phyDatdat_sak |> phyDat(type = "USER", levels = c("1", "2"))  |> View()
